...
MONITOR_DIR = "training"
ALPHA = 0.02                           #learning rate
NOISE = 0.05
assert NUM_BEST_DELTAS <= NUM_DELTAS   #NUM_DELTAS must be higher then NUM_BEST_DELTAS

class Normalizer():
    #Normalizes inputs
    def __init__(self, n_inputs):
        #empty arrays of the size of our inputs
...
class Augmented_Random_Search():
    ...
    def policy(self, state, delta = None, direction = None):
        #give it a state and  policy return action to take in that state
        if direction:
